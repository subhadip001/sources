...
public class CrashlyticsController {
    public static final FilenameFilter APP_EXCEPTION_MARKER_FILTER = new FilenameFilter() { // from class: f.h.e.i.e.c.a
    ...
    public void doWriteAppExceptionMarker(long j2) {
        try {
            ...
            if (fileStore.getCommonFile(APP_EXCEPTION_MARKER_PREFIX + j2).createNewFile()) {
                return;
            }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                StringBuilder A = a.A("Could not parse app exception timestamp from file ");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        Logger logger2 = Logger.getLogger();
        logger2.v("ANR feature enabled, but device is API " + i2);
    }
