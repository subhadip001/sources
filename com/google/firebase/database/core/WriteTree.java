...
public class WriteTree {
    ...
    public void addMerge(Path path, CompoundWrite compoundWrite, Long l2) {
        Utilities.hardAssert(l2.longValue() > this.lastWriteId.longValue());
    ...
    public void addOverwrite(Path path, Node node, Long l2, boolean z) {
        Utilities.hardAssert(l2.longValue() > this.lastWriteId.longValue());
        this.allWrites.add(new UserWriteRecord(l2.longValue(), path, node, z));
        if (z) {
    ...
    public boolean removeWrite(long j2) {
        ...
        for (int size = this.allWrites.size() - 1; isVisible && size >= 0; size--) {
            ...
            if (userWriteRecord2.isVisible()) {
                if (size >= i2 && recordContainsPath(userWriteRecord2, userWriteRecord.getPath())) {
                    isVisible = false;
