    ...
    public void logArmActive(String str, ConfigContainer configContainer) {
        ...
        if (configs.length() >= 1 && (optJSONObject = personalizationMetadata.optJSONObject(str)) != null) {
            ...
            synchronized (this.loggedChoiceIds) {
                ...
                Bundle S = a.S(EXTERNAL_RC_PARAMETER_PARAM, str);
                S.putString(EXTERNAL_ARM_VALUE_PARAM, configs.optString(str));
                S.putString(EXTERNAL_PERSONALIZATION_ID_PARAM, optJSONObject.optString(PERSONALIZATION_ID));
                S.putInt(EXTERNAL_ARM_INDEX_PARAM, optJSONObject.optInt(ARM_INDEX, -1));
                S.putString("group", optJSONObject.optString("group"));
                analyticsConnector.logEvent(ANALYTICS_ORIGIN_PERSONALIZATION, EXTERNAL_EVENT, S);
